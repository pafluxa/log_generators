...
from log_analyzer.generators.uss_enterprise import USSEnterpriseDiagnosticGenerator


class USSEnterpriseDiagnosticClassifier:
    """
    ...
    _base_model_name: str = 'bert-base-cased'
    _labels: List[str]
    _label_encoder: OrdinalEncoder = OrdinalEncoder()
    _loracfg: LoraConfig
    ...
    def system_info_to_multihot(self, systems_info: Dict):
        ...
        for system_name, system_config in self.systems.items():
            for subsystem in system_config['subsystems']:
                labels.append(f"{system_name}::{subsystem}")
        # Fit ordinal OrdinalEncoder
        self._label_encoder.fit(labels)

        batch_size = len(self._label_encoder.categories_)
    ...
    def setup_tokenizer(self):
        ...
        self.tokenizer = \
            AutoTokenizer.from_pretrained(self._base_model_name, trust_remote_code=True)

    def setup_model(self):
        self._model = AutoModelForSequenceClassification.from_pretrained(
            self._base_model_name,
             num_labels=len(self._label_encoder.categories_),
